...
function Post({ eventId, images, status, description, title, venue, date, timestamp }) {
  ...
  >
    ...
    <SoftBox pt={3} px={0.5}>
      ...
      <div><ReviewsIcon style={{color:'#2a68af'}}/>{abbrNum(b,1)}</div>
      </SoftBox>
      <hr/>
      ...
      style={{backgroundColor:'#2a68af',border:'1px solid #2a68af'}}
    >
      view
    </SoftButton>    
      </Link>
        ...
        {status === true ?(
          <span style={{fontWeight:'bold',fontSize:20, color:'#2a68af'}}><AccessTimeIcon /></span>
        ):(<span style={{fontWeight:'bold',fontSize:20, color:'#2a68af'}}><TaskIcon /></span>)}
        </SoftBox>
      </SoftBox>
    </SoftBox>
